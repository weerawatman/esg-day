...
export function createRoomStore(client, {retries = 8, delay = attempt => 20 + Math.random() * 40 * (attempt + 1), sync} = {}) {
 ...
 async function withRoom(code, run) {
  for (let attempt = 0; attempt < retries; attempt++) {
   ...
   if (await saveRoom(code, rooms[code], row.version)) {
    // Reporting is a secondary mirror for the operator's own queries; a hiccup here must
    // never fail the player's actual request.
    if (sync) await sync(code, rooms[code]).catch(e => console.error('reporting sync failed:', e.message));
    return result;
   }
