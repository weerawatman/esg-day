...
function progress(){return `<div class="question-dots" aria-label="ความคืบหน้า ${state.question.id+1} จาก ${state.total} ข้อ">${Array.from({length:state.total},(_,i)=>`<span class="${i===state.question.id?'active':i<state.question.id?'done':''}">${i+1}</span>`).join('')}<b>${state.question.round===3?'DOUBLE POINTS ×2':roundNames[state.question.round-1]}</b></div>`}
function countdown(){return `${roomHeading()}${progress()}<section class="countdown-stage"><span class="eyebrow">QUESTION ${state.question.id+1} / ${state.total}</span><h2>${state.question.points>100?'คะแนนคูณสอง! พร้อมพลิกเกม?':'ปรึกษาทีม แล้วเตรียมตอบ!'}</h2><div class="countdown-number" id="intro-count">3</div><p>${state.me?`ทีม ${state.teams[state.me.team].name} สู้ ๆ!`:'ทุกทีมพร้อมแล้วใช่ไหม?'}</p><span class="pill">ข้อนี้ ${state.question.points} คะแนน · ${state.question.seconds} วินาที</span></section>`}
...
function rules(){const dialog=document.createElement('dialog');dialog.innerHTML=`<button class="dialog-close" aria-label="ปิด">×</button><span class="eyebrow">PLAY TOGETHER. WIN TOGETHER.</span><h2>กติกาแบบไว ๆ</h2><ol><li>กรอกรหัสพนักงาน + ชื่อเล่น แล้วสุ่มเข้า 3 ทีม</li><li>ช่วยกันคิด แต่ทุกคนส่งคำตอบจากมือถือของตัวเอง</li><li>ข้อ 1–8 ตอบถูก +100 · ข้อ 9–10 คะแนนคูณสอง +200</li><li>ดูนาฬิกาถอยหลัง! หมดเวลาแล้วส่งไม่ได้</li><li>เฉลยแล้วลุ้นอันดับ ทำแต้มสะสมจนครบ 10 ข้อ</li></ol><p>จำนวนคนต่างกัน จัดอันดับด้วย % คะแนนเต็มทีม<br>ไม่มีโบนัสความเร็ว · คะแนนเท่ากันเป็นแชมป์ร่วม</p><p class="micro">พิธีกรเลือกเวลาต่อข้อได้ 15 / 20 / 30 / 45 / 60 วินาที</p>`;document.body.append(dialog);dialog.querySelector('button').onclick=()=>dialog.close();dialog.onclose=()=>dialog.remove();dialog.showModal()}
