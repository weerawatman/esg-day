...
test('solutions and histogram hidden; duplicate/stale answers rejected; reveal scored once',()=>{
 ...
 v=game.view(room.code,players[0].token);assert.equal(v.teams[2].score,100);assert.equal(v.question.correct,1);assert.deepEqual(v.distribution,[0,1,0,0]);assert.equal(v.me.gained,100);assert.throws(()=>game.control(room.code,room.hostToken,'reveal'));assert.throws(()=>game.control(room.code,room.hostToken,'next'));
...
test('ten questions, 1200 points/player; 7/7/6 teams share perfect normalized rank; separate new room',()=>{
 ...
 const v=game.view(room.code);assert.equal(v.phase,'finished');assert.equal(v.total,10);assert.equal(v.teams.length,3);v.teams.forEach(t=>{assert.equal(t.score,t.count*1200);assert.equal(t.percent,100);assert.equal(t.rank,1)});const next=game.create();assert.notEqual(next.code,room.code);assert.equal(game.view(next.code).players.length,0);assert.equal(game.view(room.code).players[0].score,1200);
});
test('session snapshot restores employee identity, team, answer and points without re-randomizing',()=>{
 const {game,room,players,start}=setup();start();game.answer(room.code,players[0].token,0,1);game.control(room.code,room.hostToken,'close');game.control(room.code,room.hostToken,'reveal');const restored=new Game({rooms:JSON.parse(JSON.stringify(game.rooms)),now:()=>4000});const v=restored.view(room.code,players[0].token);assert.equal(v.me.id,players[0].id);assert.equal(v.me.team,players[0].team);assert.equal(v.me.answer,1);assert.equal(v.me.score,100);
});
